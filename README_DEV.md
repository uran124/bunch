...
- Роутинг реализован через параметр `?page=...` (учёт ограничений shared‑хостинга).
...
Навигация может работать как обычные ссылки (`/?page=...`), опционально дополняется JS‑роутером для частичной перезагрузки.
...
- Админка: страница `?page=admin-services-delivery` отрисовывает карту Leaflet (OSM) и список зон. Можно рисовать/редактировать полигоны, задавать название, цену, приоритет, цвет и активность зоны. Кнопка «Сохранить» отправляет `POST /api/delivery/zones` и увеличивает `delivery_pricing_version` в БД.

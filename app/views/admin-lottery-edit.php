    ...
    <div class="grid gap-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm shadow-violet-50/60 ring-1 ring-transparent lg:grid-cols-[1.1fr_1fr]">
        ...
        <form action="/?page=admin-lottery-update" method="post" class="grid gap-3">
            ...
            <div class="grid gap-3 sm:grid-cols-2">
                <label class="flex flex-col gap-2 text-sm font-semibold text-slate-700">
                    Стоимость билета, ₽
                    <input type="number" name="ticket_price" min="0" step="0.01" value="<?php echo htmlspecialchars((string) ($lottery['ticket_price'] ?? 0), ENT_QUOTES, 'UTF-8'); ?>" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm text-slate-900 shadow-sm focus:border-violet-300 focus:outline-none focus:ring-2 focus:ring-violet-200">
